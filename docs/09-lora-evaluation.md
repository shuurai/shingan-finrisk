...
- **真实数据上的零样本行：没有跑。** 见 12.6 的三个选项。合成数据上它的失败原因是 prompt 契约缺口，而这个缺口在真实数据上**一模一样存在**（同一个 `SYSTEM_PROMPT`）。这是**推断，不是测量**，按本仓口径不能写成结果。（后续：选项 A 已执行，真实数据零样本行于 2026-09-26 落地，见 13.9.1——本条的"推断"状态已被测量替代。）
...
2. **适配器是表里唯一的常量。** KS 恰 0.0000、方向 `tied`，64 行**只有 1 个不同分数**（全部 `0.0`）；零样本臂在同一批行上有 **16 个不同分数**（`0.4` × 13、`0.65` × 10、`0.35` × 7 ……），正类均值 0.5022 高于负类均值 0.4882。零样本臂不是常量（KS 0.1273），而且它是**表里唯一方向为 `positives_higher` 的一行**——所有拟合基线（含 `text_baseline`）都是 `negatives_higher`。AUC 0.5172 的区间跨零，所以这**不是**"零样本更强"的结论；但它足以否定"基座模型只会输出一个数"，而那正是 Step 7 无法区分的事。
3. **`lora − zero_shot` 的估计是负的**（AUC −0.0172、PR-AUC −0.0169），区间跨零。

**独立复算（不经过本仓的指标代码）**：直接读产物 `predictions`、用 sklearn 重算得到零样本 AUC **0.517172** / PR-AUC **0.157495**、适配器 AUC **0.500000** / PR-AUC **0.140625**，与产物一致；两臂的 `y_true` 逐行相同，确认配对成立。
...
#### 13.9.1 落地：69.5 小时后的真实零样本数字（2026-09-26）
...
- **真实数据上的适配器臂没有跑。** 按 Step 5 的决定（正样本扩容之前不再为这个常量花 GPU），真实数据上只跑了零样本臂；它的三次尝试见 13.9——第三次于 2026-09-26 落地，真实零样本数字见 13.9.1。两臂的配对差值至今只在合成面板上成立。
- **"契约修好了"只在解析率上验证，没有验证引用质量。** `source_ref` 仍然**没有任何门在校验**（13.2 的第二个缺口只被"写清楚了"，没有被"检查起来"）。模型现在知道该写什么形状，但没人核对它指的文档是否真的存在。 **为什么这一条没有在本步顺手关掉**：把校验做出来只需要一个纯函数（解析出的 `source_ref` 是否等于渲染进该行 prompt 的某个块标识），不需要 GPU。但它要**落到产物里**才有人读得到，而这一步的评测**已经在跑**：事后才加字段，会让真实数据那份产物与合成数据那份产物处在**不同的保真度**上——那正是本仓已经定性为缺陷的形态（"同一个指标有两份产物时，保真度低的那份就是缺陷"）。所以它的正确落点是**下一次重跑之前**，不是这一次之后。
...
- **安慰剂对照**：脚本已备（`scripts/placebo_corpus.py`，把每个 split 的 user turn
  ...
  GPU 已空闲（真实零样本行 2026-09-26 落地，见 §13.9.1），**已执行，结果见 §15**。
- 真实零样本行已于 2026-09-26 落地（69.5 h），结果见 §13.9.1；产物为旧 schema（无引用审计）。

### 15. Step 10 — 安慰剂对照（ADR-0003 要求的实验，2026-09-26）

13.9.1 落地后 GPU 空闲，ADR-0003 要求的安慰剂对照当日执行（用户终端运行，
命令出自 docs/04 §3.2）。做法：把合成 SFT 语料每个 split 的 **user turn 固定种子置换**
（`scripts/placebo_corpus.py`，system 轮与 assistant 目标不动），用与 `lora-contract-v2`
完全相同的配方重训一次适配器，再在同一合成 test 块上评分。

- 训练：`artifacts/lora-placebo/`，108 步 / **47 min** / train_loss **0.4093**（contract-v2：
  0.3624）/ 末轮 eval_loss 0.187（contract-v2：0.1565，略高——文本被打乱后可拟合的
  信号更少，方向正确）。
- 评分：`artifacts/lora-eval/20260926T104330Z`（bs=4，合成短 prompt，3 分钟；
  `--no-verify-prompts` 是刻意的——重建 prompt 与安慰剂语料本来就该不一致）。

#### 15.1 结果：安慰剂适配器也是常量

| | contract-v2（真配对） | placebo（置换配对） |
| --- | --- | --- |
| 可解析行 | 64/64 | **36/64**（失败率 0.4375，丢弃正样本 5/9） |
| 不同分数个数 | **1** | **1**（36 行全部 0.0） |
| AUC / KS | 0.5000 / 0.0000 | **0.5000 / 0.0000** |
| KS 方向 | tied | tied |

**"微调只买到格式"从推断升级为实验结论。** 打乱文本摧毁了 prompt→target 的条件信号，
同一配方、同一训练量产出的适配器与真配对适配器一样是常量。ADR-0003 的要求满足。

另一个**更锋利**的次要发现：安慰剂适配器连**标签回显都没学会**——36 行可解析之外，
20 行答 `default_risk`、8 行答 `fraud_risk`（守卫如实拒绝）。真配对适配器是 64/64
标签正确。也就是说：真实 prompt→target 配对教会了模型"被问的是哪个标签、答案长什么样"，
打乱之后连这个都学不全。这是"微调买到了什么"最具体的答案：**条件化，而不是排序**。

#### 15.2 引用审计在安慰剂臂上的读数

`citations (text_only_lora): 0/72 resolved`——安慰剂适配器忠实复刻了训练目标的引用形状，
而训练目标的新闻引用是光秃源名（§14.5 的已知缺口），本就不可解析为任何一篇文章。
审计如设计工作：它把"教了什么形状"如实报了出来。修引用形状必须与下一次重训同批。

#### 15.3 两条不要误读的提示

- **PR-AUC 配对差 −0.0772 [−0.4331, −0.0040] 的"不跨零"不要引用。** 一个常量臂的
  bootstrap 方差近似为零，区间形状由 36 行、4 个正样本的基线方差决定——这是退化
  臂 + 小样本的假象，不是"安慰剂显著更差"。AUC 区间照常跨零。
- **本实验只支持"排序不是微调买来的"，不支持"文本没有增量"。** 后者要靠真实面板上
  的正样本扩容与两臂配对（13.9.1 的零样本基线已有实测，适配器臂仍欠）。
