...
**Status: POC with a first real-data result, and that result is provisional.** The pipeline runs end to end on **synthetic data**, which proves it is connected and nothing more. Stage 2 has now also been executed against real SEC and market data — 34 companies, one label (`tail_risk`), 39 positives of which 5 fall in the test block. Those numbers are real measurements on real filings and prices, but they rest on a sample far too small to support a performance claim, and they appear in **one row of the status table** plus the caveat list below — every ablation table in between comes from `shingan demo` on synthetic data, where the signal is planted by the generator. The SEC filing *text* corpus has since been downloaded (2,222 primary documents), the panel rebuilt with live text features, and **the evaluation re-run on that panel** — which is why `structured` moved 0.7686 → 0.6294, why the text-only PR-AUC moved 0.2400 → 0.2956, and why the comparison now carries `structured_matched` as a fourth row. Everywhere else, the numbers in the documentation are still **targets or gates**, not achieved values.
...
| [`tests/test_lora_eval.py`](tests/test_lora_eval.py) | 38 | `(sample_id, label)` as the prompt-comparison key, parse-failure taxonomy, strict JSON (`NaN` is not JSON), paired-bootstrap sign and degeneracy, adapter fingerprinting, the row set of a comparison being stated rather than only counted, and the system turn being part of the prompt check |
| [`tests/test_zero_shot.py`](tests/test_zero_shot.py) | 30 | the mode-to-arm mapping and why the adapter arm must run last, the comparison-grouping plan, model-identity resolution and refusal (including two disagreeing base models), refusing a bad call *before* importing `peft`/`torch`, and the zero-shot artifact's three disclosures |
...
| `cli.py` | 22% (publish and doctor paths covered) |
...
| QLoRA training | Implemented · verified for the *link* — `artifacts/lora-contract-v2/adapter/` is the current completed 108-step 14B adapter. The earlier `artifacts/lora/` was trained under the old instruction contract; it is kept rather than overwritten, with `SUPERSEDED.md` beside it recording why the prompt gate now refuses to score it (a refusal that is the gate working, not damage). **The current adapter's output is constant under in-distribution evaluation** (AUC exactly 0.5000, every row the same score), so it is evidence that the stack works on 32 GB, not evidence of predictive power |
